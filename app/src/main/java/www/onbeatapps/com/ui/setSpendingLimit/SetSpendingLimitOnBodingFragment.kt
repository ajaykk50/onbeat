...
@AndroidEntryPoint
class SetSpendingLimitOnBodingFragment : BaseFragment<SpendingLimitViewModel>() {
    ...
    override fun setup() {
        ...
        spendingLimitViewModel.setLowBalance("")
...
//        stripePay()
    ...
    private fun displayContent(type: String) {
        ...
        binding.apply {
            ...
            if (type == "SPENT") {
                txtTile.typeface = mContext.resources.getFont(R.font.os_font_regular)
                txtMainContent.typeface = mContext.resources.getFont(R.font.bahnschrif)
                btSkip.text = "Skip for now"
                containPreauth.visibility = View.GONE
                containSpend.visibility = View.VISIBLE
                btSkip.setBackgroundDrawable(mContext.getDrawable(R.drawable.bg_white_blue_line))
                btSkip.setTextColor(Color.parseColor("#51AADF"))
                btNext.setBackgroundDrawable(mContext.getDrawable(R.drawable.ic_bg_gradient_small))
                btNext.setTextColor(Color.parseColor("#FFFFFF"))
            }else{
                containPreauth.visibility = View.VISIBLE
                containSpend.visibility = View.GONE
                txtContent.text = "This will settle your existing pre-authorisation, debiting ${spendingLimitViewModel.Currency()}${NumberFormatForCurrencyCode(spendingLimitViewModel.getUsedAmount().toString())} from your card.\n\nTop up will pre-authorise ${spendingLimitViewModel.Currency()}${NumberFormatForCurrencyCode(spendingLimitViewModel.getTemAmount())} on your card and your new wallet balance will then be ${spendingLimitViewModel.Currency()}${NumberFormatForCurrencyCode(spendingLimitViewModel.getTemAmount())}"
            }
        }
    }
    ...
    private fun setUpRecy() {
        binding.apply {
            ...
            spendingAdapter = SpendingLimitAdapter(mContext, spendingLimitViewModel) {
                refreshamout()
                amount = it.toString()
                spendingLimitViewModel.setTempAmount(amount)
            ...
            spendingAdapter!!.setListContent(0)
        }
        ...
        binding.apply {
            ...
            spendingLimitViewModel.cardDt.observe(viewLifecycleOwner) {
                if (it.card.isNotEmpty()) {
                    ...
                    spendingLimitViewModel.addSpendLimit("${spendingLimitViewModel.getTemAmount()}")
//
                }
            }
            ...
            if (it) {
                if (!spendingLimitViewModel.getCardStatus()) {
                    findNavController().navigate(
                        ...
                        bundleOf(HIDE_COUNT to hideCount,"Amount" to spendingLimitViewModel.getTemAmount())
                    )
                } else {
                    callPaySheet()
                }
                ...
                if (amount != "" && amount.isNotEmpty()) {
                    if (amount.toInt() < 20) {
                        ...
                        displayContent("PREE_AUTH")
                            ...
                            if (spendingLimitViewModel.getCardStatus())
                                spendingLimitViewModel.getListCard(spendingLimitViewModel.getTemAmount())
                            else
                                spendingLimitViewModel.addSpendLimit("${spendingLimitViewModel.getTemAmount()}")
                        }
                    }
